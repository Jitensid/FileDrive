...
const FileOptions = (props) => {
	...
	const makeRequest = (filename, fileindex, newstatus) => {
		trackPromise(
			AxiosApiInstance.AxiosApiInstance.post(
				'api/changefilestarstatus/',
				{
					filename: filename,
				}
			).then(() => {
	...
	const show_valid_menu_option = (fileelement) => {
		if (fileelement.is_starred) {
			return (
				<MenuItem
					onClick={() =>
						removefromStarredFiles(fileelement.filename)
					}>
	...
	return (
		<React.Fragment>
			...
			<Menu
				...
				<MenuItem
					onClick={() => deleteFile(props.uploadedfile.filename)}>
					Delete
				</MenuItem>
			</Menu>
		</React.Fragment>
